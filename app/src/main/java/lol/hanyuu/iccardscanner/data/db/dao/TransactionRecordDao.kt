...
interface TransactionRecordDao {
    @Query("SELECT * FROM transaction_records WHERE cardIdm = :idm ORDER BY transactionDate DESC, sequence DESC")
    fun getByCard(idm: String): Flow<List<TransactionRecordEntity>>

    @Query("SELECT * FROM transaction_records WHERE cardIdm = :idm ORDER BY transactionDate DESC, sequence DESC LIMIT :limit")
